...
describe('OAuthAuthenticatedGuard', () => {
    ...
    const createService: SpectatorServiceFactory<OAuthAuthenticatedGuard> = createServiceFactory({
    ...
    it('should return false when the user is not authenticated', async () => {
        ...
        const canActivate: boolean | UrlTree = await spectator.service.canActivate().toPromise();
